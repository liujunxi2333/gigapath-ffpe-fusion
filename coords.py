...
# 不在模块顶层 import openslide：部分节点上 OpenSlide/libtiff 会在 import 时段错误，
# 与是否使用 tifffile 扫描无关；仅在需要 OpenSlide 的函数内延迟导入。
...
def compute_tissue_coords_tifffile_shaped(
    slide_path: str,
    tile_size: int,
    target_level: int,
    bg_threshold: int,
    ...
    """
    用 tifffile 读整幅（常见为单层级 RGB），不经 OpenSlide。

    用于 OpenSlide/libtiff 在 ``read_region`` 上触发段错误、但 tifffile 能解码的 .tif。
    内存占用 ≈ 高×宽×通道 字节；极大图请保证作业 ``--mem`` 足够。
    ``target_level`` 当前仅按 level0（倍率 1）处理，与默认 ``--target_level 0`` 一致。
    """
    import tifffile
    ...
    if arr.ndim == 2:
        thumb_gray = arr.astype(np.float32)
    elif arr.ndim == 3:
        thumb_gray = np.mean(arr, axis=2, dtype=np.float64).astype(np.float32)
    else:
        raise ValueError(f"tifffile 组织扫描暂不支持数组维数: {arr.shape}")

    slide_h0, slide_w0 = int(thumb_gray.shape[0]), int(thumb_gray.shape[1])
    # 单层整图：与 level0 对齐，缩略倍率为 1
    _ = target_level
    ...
    grid_x0 = (x0 // level0_tile_size) * level0_tile_size
    grid_y0 = (y0 // level0_tile_size) * level0_tile_size
    valid = (grid_x0 + level0_tile_size <= slide_w0) & (
        grid_y0 + level0_tile_size <= slide_h0
    )
    if not np.any(valid):
        return [], time.perf_counter() - t0
    coords = np.stack([grid_x0[valid], grid_y0[valid]], axis=1)
